...
def train_online(student, student_pre, pa_manager, train_loader, test_loader, session, args):
    ...
    cluster_crit = DistillLoss(args.warmup_teacher_temp_epochs, args.epochs_online_per_session, args.n_views, args.warmup_teacher_temp, args.teacher_temp)
    best_acc = 0
    for epoch in range(args.epochs_online_per_session):
        ...
        for batch in train_loader:
            ...
            # 基础损失
            cluster_loss = cluster_crit(out, out.detach(), epoch)
            avg_p = (out/0.1).softmax(1).mean(0); m_old, m_new = torch.sum(avg_p[:args.num_seen_classes]), torch.sum(avg_p[args.num_seen_classes:])
            ...
            # --- RCTD (方案 2) 核心改进：关系一致性拓扑蒸馏 ---
            feats = F.normalize(student[0](img), -1)
            with torch.no_grad(): f_pre = F.normalize(student_pre[0](img), -1)
            
            # 计算样本间相似度矩阵 (拓扑结构)
            rel_s = torch.matmul(feats, feats.t())
            rel_t = torch.matmul(f_pre, f_pre.t())
            
            # 使用 MSE 保持拓扑结构一致性
            distill_loss = F.mse_loss(rel_s, rel_t)
            # -----------------------------------------------
