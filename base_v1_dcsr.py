...
feature_extract_dir = '/lustre/home/sjma/Continual-GCD-project/GCD-baseline/extracted_features_public_impl'
exp_root_happy = '/home/asus/Happy-CGCD3/dev_outputs_Happy'  

class ProtoAugManager:
    """
    DCSR (方案 1): 分布校准语义回放
    核心改进：存储每个类别的独立方差（迹半径），生成更符合真实分布的虚拟特征。
    """
    def __init__(self, feature_dim, batch_size, hardness_temp, radius_scale, device, logger):
        ...
        self.radius = None # 修改：存储每个类别的独立半径 Tensor [num_classes]
    ...
    def compute_proto_aug_hardness_aware_loss(self, model):
        ...
        # 核心改进：根据采样的标签，提取对应类别的特定半径进行高斯采样
        current_radius = self.radius[prototypes_labels].unsqueeze(1).to(self.device)
        prototypes_augmented = prototypes_sampled + torch.randn((self.batch_size, self.feature_dim), device=self.device) * current_radius * self.radius_scale
    ...
    def update_prototypes_offline(self, model, train_loader, num_labeled_classes):
        ...
        for batch_idx, (images, label, _) in enumerate(tqdm(train_loader)):
            ...
            with torch.no_grad():
                feats = torch.nn.functional.normalize(model[0](images), dim=-1)
                all_feats_list.append(feats)
                all_labels_list.append(label)
        all_feats = torch.cat(all_feats_list, dim=0)
        all_labels = torch.cat(all_labels_list, dim=0)

        prototypes_list, radius_list = [], []
        for c in range(num_labeled_classes):
            ...
            # 计算类内迹方差 (Class-specific Radius)
            feats_c_center = feats_c - feats_c_mean
            cov = torch.matmul(feats_c_center.t(), feats_c_center) / len(feats_c_center)
            radius_c = torch.trace(cov) / self.feature_dim
            radius_list.append(radius_c)
            
        self.radius = torch.stack(radius_list).to(self.device)
        self.prototypes = F.normalize(torch.stack(prototypes_list), dim=-1, p=2)

        similarity = self.prototypes @ self.prototypes.T
        for i in range(len(similarity)): similarity[i,i] = 0
        self.mean_similarity = torch.sum(similarity, dim=-1) / (len(similarity)-1)

    def update_prototypes_online(self, model, train_loader, num_seen_classes, num_all_classes):
        model.eval()
        all_preds_list, all_feats_list = [], []
        ...
        for c in range(num_seen_classes, num_all_classes):
            ...
            if len(feats_c) <= 1:
                ...
                radius_c = torch.mean(self.radius) # 使用已知类的平均半径作为回退
            else:
                feats_c_mean = torch.mean(feats_c, dim=0)
                feats_c_center = feats_c - feats_c_mean
                cov = torch.matmul(feats_c_center.t(), feats_c_center) / len(feats_c_center)
                radius_c = torch.trace(cov) / self.feature_dim
            p_list.append(feats_c_mean)
            r_list.append(radius_c)
            
        self.prototypes = torch.cat([self.prototypes, torch.stack(p_list)], dim=0)
        self.prototypes = F.normalize(self.prototypes, dim=-1, p=2)
        self.radius = torch.cat([self.radius, torch.stack(r_list).to(self.device)], dim=0)
...
def info_nce_logits(features, n_views=2, temperature=1.0, device='cuda'):
    ...
    labels = torch.cat([torch.arange(b_) for _ in range(n_views)], dim=0).to(device)
...
def get_kmeans_centroid_for_new_head(model, loader, args, device):
    ...
    with torch.no_grad():
        for images, _, _, _ in tqdm(loader):
            feats = F.normalize(model[0](images.cuda()), dim=-1)
    ...
    with torch.no_grad():
        _, logits = model[1](centroids)
        max_logits, _ = torch.max(logits, dim=-1)
        _, proto_idx = torch.topk(max_logits, k=args.num_novel_class_per_session, largest=False)
    return centroids[proto_idx]

def train_offline(student, train_loader, test_loader, args):
    optimizer = SGD(get_params_groups(student), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs_offline, eta_min=args.lr * 1e-3)
    cluster_criterion = DistillLoss(args.warmup_teacher_temp_epochs, args.epochs_offline, args.n_views, args.warmup_teacher_temp, args.teacher_temp)
    best_acc = 0
    for epoch in range(args.epochs_offline):
        student.train(); loss_record = AverageMeter()
        for batch_idx, batch in enumerate(train_loader):
            images, labels, _ = batch
            labels = labels.cuda(non_blocking=True); images = torch.cat(images, dim=0).cuda(non_blocking=True)
            proj, out = student(images)
            cls_loss = nn.CrossEntropyLoss()(torch.cat([f for f in (out / 0.1).chunk(2)], 0), torch.cat([labels for _ in range(2)], 0))
            cluster_loss = cluster_criterion(out, out.detach(), epoch)
            avg_probs = (out / 0.1).softmax(dim=1).mean(dim=0)
            me_max_loss = - torch.sum(torch.log(avg_probs**(-avg_probs))) + math.log(float(len(avg_probs)))
            cluster_loss += args.memax_weight * me_max_loss
            c_logits, c_labels = info_nce_logits(proj)
            contrast_loss = nn.CrossEntropyLoss()(c_logits, c_labels)
            student_proj = torch.nn.functional.normalize(torch.cat([f.unsqueeze(1) for f in proj.chunk(2)], dim=1), dim=-1)
            sup_con_loss = SupConLoss()(student_proj, labels=labels)
            loss = (1 - args.sup_weight) * (cluster_loss + contrast_loss) + args.sup_weight * (cls_loss + sup_con_loss)
        ...
        if old_acc > best_acc:
            best_acc = old_acc
            torch.save({'model': student.state_dict()}, args.model_path[:-3] + '_best.pt')
...
def test_offline(model, loader, epoch, name, args):
    ...
    for img, lab, _ in tqdm(loader):
        with torch.no_grad():
            ...
            p.append(out.argmax(1).cpu().numpy()); t.append(lab.cpu().numpy())
...
def train_online(student, student_pre, pa_manager, train_loader, test_loader, session, args):
    optimizer = SGD(get_params_groups(student), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs_online_per_session, eta_min=args.lr * 1e-3)
    cluster_criterion = DistillLoss(args.warmup_teacher_temp_epochs, args.epochs_online_per_session, args.n_views, args.warmup_teacher_temp, args.teacher_temp)
    best_acc = 0
    for epoch in range(args.epochs_online_per_session):
        student.train(); student_pre.eval(); loss_record = AverageMeter()
        for batch_idx, batch in enumerate(train_loader):
            images, labels, _, _ = batch
            labels = labels.cuda(); images = torch.cat(images, 0).cuda()
            proj, out = student(images)
            cluster_loss = cluster_criterion(out, out.detach(), epoch)
            avg_p = (out / 0.1).softmax(1).mean(0)
            m_old, m_new = torch.sum(avg_p[:args.num_seen_classes]), torch.sum(avg_p[args.num_seen_classes:])
            cluster_loss += args.memax_old_new_weight * (m_old * torch.log(m_old+1e-6) + m_new * torch.log(m_new+1e-6) + math.log(2))
            pa_loss = pa_manager.compute_proto_aug_hardness_aware_loss(student)
            feats = student[0](images)
            with torch.no_grad(): f_pre = student_pre[0](images)
            distill_loss = (F.normalize(feats, -1) - F.normalize(f_pre, -1)).pow(2).sum() / len(feats)
            loss = cluster_loss + args.proto_aug_weight * pa_loss + args.feat_distill_weight * distill_loss
            optimizer.zero_grad(); loss.backward(); optimizer.step()
            loss_record.update(loss.item(), labels.size(0))
        res = test_online(student, test_loader, epoch, 'Test', args)
        scheduler.step()
        if res[0] > best_acc:
            best_acc = res[0]
            torch.save({'model': student.state_dict()}, args.model_path[:-3] + f'_session-{session}_best.pt')
